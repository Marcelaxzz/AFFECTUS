...
export default function JogoMatematico() {
  ...
  const [respostaClicada, setRespostaClicada] = useState<number | null>(null); // FLAG PARA RESPOSTA CLICADA
  const [somAtivo, setSomAtivo] = useState(true); // FLAG PARA SOM ATIVO
  const [mostrarConquista, setMostrarConquista] = useState<null | string>(null); // FLAG PARA MOSTRAR CONQUISTA

  // CALCULA O PROGRESSO DO JOGO EM PORCENTAGEM
  const progresso = ((indice + 1) / perguntas.length) * 100;
const navigate = useNavigate();
  // FUNCAO PARA TOCAR SOM
  function tocarSom(src: string | undefined) {
    if (somAtivo) {
  ...
  function verificarResposta(opcao: number | null) {
      ...
      <div className="container-jogo">
        ...
        <div className="header-jogo">
          <div className="botoes-header">
            <button className="btn-sair" onClick={() => navigate("/GamesScreen")}>
              ⮜
            </button>
