...
export default function JogoReflexo() {
  const [blocoAtivo, setBlocoAtivo] = useState<number | null>(null);
  const [pontuacao, setPontuacao] = useState(0);
  const [vidas, setVidas] = useState(5);
  const [jogando, setJogando] = useState(false);
  const [mostrarModal, setMostrarModal] = useState(false);
  const [animacaoBloco, setAnimacaoBloco] = useState<{ index: number; tipo: "correto" | "errado" } | null>(null);
  ...
  // Clique nos blocos
  function clicarBloco(index: number | null) {
    if (!jogando || bloqueandoClickRef.current || index === null) return;
